...
export async function updateActivity(activityId: number, formData: FormData): Promise<ActivityFormResult> {
  const session = await getServerSession(authOptions)
  if (!session?.user) {
    return { success: false, error: "Neautorizat" }
  }
  ...
  if (!existingActivity) {
    return { success: false, error: "Activitate negasita sau nu ai permisiune" }
  }
  ...
  await logAudit({ userId: session.user.id, action: "update", tableAffected: "activities", recordId: activity.id, details: { date: activity.date.toISOString(), durationMin, sport, trimp } })
