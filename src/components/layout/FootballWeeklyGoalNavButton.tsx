...
import { setFootballTrainingWeeklyGoal } from "@/app/(dashboard)/actions/football-weekly-goal"

type FootballWeeklyGoalNavButtonProps = {
    label: string
    isActive?: boolean
    weekStart?: string
    weekLabel?: string
...
export default function FootballWeeklyGoalNavButton({
    label,
    isActive = false,
    weekStart,
    weekLabel,
    targetTrimp,
}: FootballWeeklyGoalNavButtonProps) {
    ...
    async function handleSubmit(event: React.FormEvent) {
        ...
        try {
            const result = await setFootballTrainingWeeklyGoal({
                weekStart,
        ...
        <>
            ...
            {isOpen && (
                <FitnessWeeklyGoalModal
                    title="Fotbal Training Weekly Goal"
                    description="Seteaza targetul TRIMP pentru antrenamentele de fotbal ale echipei in saptamana curenta."
                    inputLabel="Target TRIMP fotbal"
